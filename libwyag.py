...
# This creates the main CLI parser
# ./wyag --help gives "The stupidest content tracker"
argparser = argparse.ArgumentParser(description="The stupidest content tracker")

# add_subparsers() tells argparse that this program has subcommands like git
# dest="command" means: Whatever subcommand the user types will be stored in args.command
# required = True forces the user to supply a subcommand
...
class GitRepository (object):
    ...
    worktree = None # worktree directory
    gitdir = None # .git directory
    conf = None # config file directory (.git/config) it is an INI file (A simple text config file with [sections], key = value pairs)
    ...
    def __init__(self, path, force=False):
        ...
        self.conf = configparser.ConfigParser() # Creates a ConfigParser object to read INI files.
...
def repo_create(path):
    ...
    ## Use 'with' to open the file safely; automatically closes the file when done
...
def object_find(repo, name, fmt=None, follow=True):
    ...
    while True:
        ...
        # Follow tags
        # tags can point to other tags, or to commits
        if obj.fmt == b'tag':
...
"""
A Git commit is an immutable object whose SHA-1 hash is computed from its
entire contents: the filesystem snapshot (tree), parent commit(s), author
and committer metadata, timestamps, message, and optional signature.
Because each commit references its parent(s), it is cryptographically
linked to the full history of the repository and can never be modified
without creating a new commit.
"""

class GitTreeLeaf (object):
    def __init__(self, mode, path, sha):
        self.mode = mode # A file mode in a Git tree is a numeric code that tells Git what kind of filesystem object it is and what permissions it has.
        self.path = path
        self.sha = sha
...
A tree object is just a sequence of entries, all stored one after another:

[mode] [space] [path] [null byte] [20-byte SHA]
[mode] [space] [path] [null byte] [20-byte SHA]
[mode] [space] [path] [null byte] [20-byte SHA]
...

So the same structure repeats over and over.
Each record = one file or directory entry
"""
...
# Recursively list the contents of a Git tree object (like `git ls-tree`)
